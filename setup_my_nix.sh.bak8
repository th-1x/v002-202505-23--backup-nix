...
let
  # Determine which PHP to use. pkgs.php is usually an alias to the default version.
  phpEnv = pkgs.php; 
...
in
{
  ...
  home.packages = [ 
    phpEnv             # This will install the PHP interpreter
    phpEnv.packages.composer # Access composer via the selected PHP environment
    pkgs.nnn 
  ];
...
apply_home_manager_config() {
    ...
    local backup_suffix="hm-script-bak" 
...
main() {
    ...
    if ! apply_home_manager_config; then
        ...
        echo "   home-manager switch --flake \"$HOME/.config/home-manager#$CONFIGURED_USERNAME\" --show-trace -b some_backup_suffix" >&2
        echo "" >&2
        echo "   (The '--extra-experimental-features' flags should be picked up from '$HOME/.config/nix/nix.conf' in a new shell session)." >&2
        echo "   (The '-b some_backup_suffix' will backup conflicting files like ~/.bashrc to ~/.bashrc.some_backup_suffix)." >&2
        echo "--------------------------------------------------------------------------------" >&2
        exit 1
    fi
    ...
    echo "have been backed up with the suffix '.${backup_suffix}' in your home directory." >&2
